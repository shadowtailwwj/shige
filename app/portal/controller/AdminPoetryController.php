...
class AdminPoetryController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $result = $this->validate($data, 'AdminPoetry');
            ...
            $res = Db::name('portal_poetry')->insert($data);;

            if ($res!=1) {
                $this->error('添加失败!');
            }

            $this->success('添加成功!', url('AdminPoetry/index'));
        }
    ...
    public function edit()
    {
        ...
        $post = Db::name('portal_poetry')->where('poetry_id',$id)->find();
    ...
    {
        $id = $this->request->param('poetry_id', 0, 'intval');
        if ($this->request->isPost()) {
            $data = $this->request->param();
            ...
            $data['update_time'] = time();
            $res = Db::name('portal_poetry')->where('poetry_id',$id)->update($data);;

            if ($res!=1) {
                $this->error('修改失败!');
            }

            $this->success('修改成功!', url('AdminPoetry/index'));
        }
    }
