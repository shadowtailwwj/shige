...
class AdminStoryController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $result = $this->validate($data, 'AdminStory');
            ...
            $res = Db::name('portal_story')->insert($data);;

            if ($res!=1) {
                $this->error('添加失败!');
            }

            $this->success('添加成功!', url('AdminStory/index'));
        }
    ...
    public function edit()
    {
        ...
        $post = Db::name('portal_story')->where('id',$id)->find();
    ...
    public function editPost()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data['update_time'] = time();
            $res = Db::name('portal_story')->where('id',$id)->update($data);;

            if ($res!=1) {
                $this->error('修改失败!');
            }

            $this->success('修改成功!', url('AdminStory/index'));
        }
    }
